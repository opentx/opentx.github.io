...
- FrSky X7 : GVARS are now displayed as Gx as oposed to GVx
...
- Horus : improvement to plane wizar (graphics,speed and memory footprint)
- Horus : people that do not like wizard can remove or rename the folder to disable the functionnality
- Horus : limit in number of themes / layouts / widgets now removed
- Companion has been significatly reworked. While the general aspect remais the same, the internal have been massively improved
...
- Companion : early versions of model tranfers between radios wth different capabilities
...
OpenTX RC9 will be available very soon
