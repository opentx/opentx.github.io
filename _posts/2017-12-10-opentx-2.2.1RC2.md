...
**MUST READ** : Binding Options is no longer a firmware choice, it is now part of baseline OpenTX. It means then when you are binding with a suitable module, a popup will appear when binding, asking you to make a choice on telemetry and channels available on receiver output pins (not SBUS)
