...
Xlite uses a '+' pad for trims. One set of trims can be accessed directly with teh '+', the second set needs '+' and pushing 'shift' at the same time. Depending on your radio mode, this can be convinient or not at all. We have introduced a solution that enable Aileron and Elevator of your actual radio mode to be always the default '+' usage. Unfortunatly, the firmware that FrSky included in their first batch(es) of Xlite does NOT include this feature, so this is a fairly big usage difference between shipped firmware and actual OpenTx  2.2.2.
