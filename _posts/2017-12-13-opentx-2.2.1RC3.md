...
**MUST READ** : CHANGES ON RESET function. There as been a major change between 2.2.0 and 2.2.1x on reset fonction. In the past, the reset function would be called only once. In 2.2.1x, the reset is executed periodicaly while the reset condition remains true. It does NOT means however that the target value will stay at 0 during that period, just that it will be periodicaly reset.
