...
- Doing a "Reset Flight" on Horus will hang the radio if there are switch and/or throttle warning, with erratic servo movement
- Resetting a line on the Outputs screen on Horus resets the wrong line
- Pushing trims into the "Extended" range on Horus (with extended trims enabled of course) while on the home screen will cause a GUI freeze
- The "Increment GVAR" SF increments as long as the function is active instead of only once
- The "Reset" SF will cause a reboot if triggered with a "continuous" state (instead of an Edge LS)
